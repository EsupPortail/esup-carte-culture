...
UPDATE etudiant SET textsearchable_index_col = setweight(to_tsvector('simple', coalesce(nom,'')), 'A')||setweight(to_tsvector('simple', coalesce(eppn,'')), 'A')||setweight(to_tsvector('simple', coalesce(csn,'')), 'A') ||setweight(to_tsvector('simple', coalesce(prenom,'')), 'B');CREATE FUNCTION textsearchable_etudiant_trigger() RETURNS trigger AS $$ begin new.textsearchable_index_col := setweight(to_tsvector('simple', coalesce(new.nom,'')), 'A')||setweight(to_tsvector('simple', coalesce(new.eppn,'')), 'A')||setweight(to_tsvector('simple', coalesce(new.csn,'')), 'A') ||setweight(to_tsvector('simple', coalesce(new.prenom,'')), 'B'); return new; end $$ LANGUAGE plpgsql;
...
INSERT INTO appli_version (id, carte_culture_version, version) SELECT nextval('hibernate_sequence'), '0.1.x', '1' WHERE NOT EXISTS (SELECT * FROM appli_version);
